function sendReport_(variables) {
  var status = JSON.parse(PropertiesService.getScriptProperties().getProperty("status"));
  ...
  PropertiesService.getScriptProperties().setProperty("status", JSON.stringify(status));
  
  var newTrigger = ScriptApp.newTrigger('activityReport').timeBased().everyDays(1).atHour(1).create();
